...
le = LabelEncoder()
classes = list(raw.columns)
for i in range(len(classes)):
    raw[classes[i]] = le.fit_transform(raw[classes[i]])
    
ob = pd.read_csv(OBSERVE_PATH)
le = LabelEncoder()
classes = list(ob.columns)
for i in range(len(classes)):
    ob[classes[i]] = le.fit_transform(ob[classes[i]])

raw_train = raw.values.tolist()
ob_test = ob.values.tolist()
train = raw_train
test = ob_test
#n_folds = 10
max_depth = 30720
...
pred = decision_tree(train, test, max_depth, min_size)
...
#scores = evaluate_algorithm(train, decision_tree, n_folds, max_depth, min_size)
